...
public class DBManager {
 ...
 //get All access record
    public ArrayList<Time> getAllTime(String ID) throws SQLException
    {
        ArrayList<Time> tim = new ArrayList<Time>();
        ResultSet rs = st.executeQuery("SELECT * from ISDUSER.LOGINTIME1 where USERID = '"+ID+"' ");
       
        while (rs.next() ) 
        {
            Time time = new Time(rs.getString("USERID"), rs.getTimestamp("LOGINTIME"), rs.getTimestamp("LOGOUTTIME"));
            tim.add(time);
...
//    }
    //Delete record in db
    public void deleteRecord(String ID, Timestamp loginT) throws SQLException 
    {
        
        
            st.executeUpdate("Delete from ISDUSER.LOGINTIME1 WHERE USERID ='"+ID+"' AND  LOGINTIME= '"+loginT+"' ");
        
      

    }
     //Find record by times in the database
    public ArrayList<Time> findRecord(String ID ,Timestamp a, Timestamp b) throws SQLException 
    {
        ...
        
    {
        ...
        ResultSet rs = st.executeQuery("SELECT * from ISDUSER.LOGINTIME1 where USERID = '"+ID+"' AND LOGINTIME between '"+a+"' and '"+b+"' ");
